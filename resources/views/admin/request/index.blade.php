    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <button id="approveSelected" class="btn bg-green-500 text-white px-4 py-2 rounded">
                    Approve Selected
                </button>
            </div>
...
$(document).ready(function() {
        ...
        columns: [
            {
                ...
                render: data => {
                    if (data.status === 'pending') {
                        return `<input type="checkbox" class="select-request" value="${data.id}">`;
                    }
                    return '';
                }
            },
    ...
    $('#selectAll').on('change', function() {
        $('.select-request').prop('checked', this.checked);
    });

    // Approve Selected button
    function approveRequests(ids, index = 0) {
        if (index >= ids.length) {
            toastr.success(`${ids.length} request(s) approved successfully.`);
            table.ajax.reload();
            return;
        }

        let id = ids[index];
        $.post(`/admin/pending/${id}/approve`, {_token: '{{ csrf_token() }}'}, function(res) {
            approveRequests(ids, index + 1); // recursive call for next request
        }).fail(function() {
            toastr.error('Failed to approve request ID ' + id);
            approveRequests(ids, index + 1);
        });
    }

    $('#approveSelected').on('click', function() {
        let ids = $('.select-request:checked').map(function() { return $(this).val(); }).get();
        ...
        if (confirm(`Approve ${ids.length} selected request(s)?`)) {
            approveRequests(ids);
        }
    });
});
